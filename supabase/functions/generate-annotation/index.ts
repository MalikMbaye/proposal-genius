...
serve(async (req) => {
  ...
  try {
    ...
    const annotationSystemPrompt = `You are an expert proposal strategist analyzing winning business proposals. Your job is to provide "game film" style annotations that explain WHY each section of a proposal works.
...
Keep annotations between 150-300 words. Use markdown formatting for emphasis.`;
    ...
    const annotationContent = aiResponse.content?.[0]?.text;
