...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are a document text extractor. Your job is to clean up and organize raw text extracted from a document.
...
- If the text appears to be a business document, RFP, brief, or professional content, preserve all relevant business details`;
    ...
    const cleanedText = aiResponse.content?.[0]?.text || extractedText;
